...
// Parte 3
class GitHub{
    static getRepositories = async (repo:string)=>{
        try{
            let result = await axios.get('https://api.github.com/repos/'+repo);
            result = await result.data;
            console.log(result);
        }catch(err){
...
const buscaUsuario = async (usuario:string) => {
    try{
        let result = await axios.get('https://api.github.com/users/'+usuario);
        result = await result.data;
        console.log(result);
    }catch(err){
